...
function Sendemail_hotel_approvel($email, $fname ,$Mobile_No, $Hotel_Date, $Hotel_Name, $Total_room, $Payment_Id, $Total_Price)
{
  ...
  try {
    ...
    $mail->isHTML(true);
    $mail->Subject = ' Congratulations Your Hotel Book has been Approved..!';
    $mail->Body    = "<h3>Welcome " . h($fname) . " to Travel_India.com</h3>
                      <h3> Your Hotel Booking has been Successfully Approved..!</h3>
                      ...
                      <body>
                          <div class='payment-slip'>
                              ...
                              <table>
                                  ...
                                  <tr><th>Hotel Date</th><td>" . h($Hotel_Date) . "</td></tr>
                                  <tr><th>Hotel Name</th><td>" . h($Hotel_Name) . "</td></tr>
                                  <tr><th>Total Rooms</th><td>" . h($Total_room) . "</td></tr>
    ...
    error_log("PHPMailer error in hotel booking approval: " . $mail->ErrorInfo);
  }
}
...
if (isset($_POST['approve'])) {
  ...
  $stmt_details = $conn->prepare("SELECT Mobile_No, date, Hotel_Name, Price FROM hotel_booking WHERE id = ? LIMIT 1");
  ...
  if ($row_details) {
    ...
    $Hotel_Date  = $row_details['date'];
    $Hotel_Name  = $row_details['Hotel_Name'];
    $Total_Price = $row_details['Price'];
    $Total_room  = "1"; // Default fallback
    $Payment_Id  = "HPAY-" . bin2hex(random_bytes(6));

    $stmt_update = $conn->prepare("UPDATE hotel_booking SET status = 'Approved' WHERE id = ?");
    $stmt_update->bind_param("i", $id);
    
    if ($stmt_update->execute()) {
      Sendemail_hotel_approvel($email, $fname ,$Mobile_No, $Hotel_Date, $Hotel_Name, $Total_room, $Payment_Id, $Total_Price);
      echo "<script>alert('Your Package Approved Successfully..!'); window.location.href='hotel_book.php';</script>";
      exit;
    } else {
...
if (isset($_POST['delete'])) {
  $id = (int)($_POST['id'] ?? 0);
  $stmt_delete = $conn->prepare("DELETE FROM hotel_booking WHERE id = ?");
  $stmt_delete->bind_param("i", $id);
  
  if ($stmt_delete->execute()) {
    echo "<script>alert('Your Package Deleted Successfully..!'); window.location.href='hotel_book.php';</script>";
    exit;
  } else {
...
// Fetch pending securely
$pending_stmt = $conn->prepare("SELECT id, user_Id, Hotel_Name, User_Name, Email_Id, Mobile_No, date, Duration, created_booking, Price FROM hotel_booking WHERE status = 'pending' ORDER BY user_Id, id");
$pending_stmt->execute();
$pending_result = $pending_stmt->get_result();

// Fetch approved list securely
$all_stmt = $conn->prepare("SELECT user_Id, Hotel_Name, User_Name, Email_Id, Mobile_No, Hotel_Address, date, Duration, created_booking, Price, Status FROM hotel_booking");
$all_stmt->execute();
      ...
      <table>
        ...
        while ($row = $pending_result->fetch_assoc()) { ?>
          <tr>
            ...
            <th scope="row"><?php echo h($row['user_Id']); ?></th>
            <td><?php echo h($row['Hotel_Name']); ?></td>
            <td><?php echo h($row['User_Name']); ?></td>
            <td><?php echo h($row['Email_Id']); ?></td>
            <td><?php echo h($row['Mobile_No']); ?></td>
            <td><?php echo h($row['date']); ?></td>
            <td><?php echo h($row['Duration']); ?></td>
            <td><?php echo h($row['created_booking']); ?></td>
            <td><?php echo h($row['Price']); ?> Rs</td>
            <td>
      ...
      <table>
        ...
        while ($row = $all_result->fetch_assoc()) { ?>
          <tr>
            ...
            <td><?php echo h($row['Email_Id']); ?></td>
            <td><?php echo h($row['Mobile_No']); ?></td>
            <td><?php echo h($row['Hotel_Address']); ?> </td>
            <td><?php echo h($row['date']); ?></td>
            <td><?php echo h($row['Duration']); ?></td>
            <td><?php echo h($row['created_booking']); ?></td>
            <td><?php echo h($row['Price']); ?> Rs</td>
